...
export type Transaction = { id: number; type: 'income' | 'expense'; category: string; amount: number; month: string }
...
export type Data = { goals: Goal[]; habits: Record<string, boolean[]>; schedule: Record<string, DayNote[]>; logs: Record<string, DayLog>; transactions: Transaction[]; financeColumns: string[]; habitNames: string[]; scheduleTimes: string[]; userName: string; monthResetKey: string }
