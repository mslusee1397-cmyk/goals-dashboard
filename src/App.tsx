...
function Dashboard({ user, logout }: { user: { id: string; username: string }; logout: () => void }) {
  ...
  const habitStats = data.habitNames.map(name => ({ name, current: getHabitStreak(data.habits[name] || []), best: getBestHabitStreak(data.habits[name] || []) }))
  ...
  // Расписание — постоянный шаблон недели. Оно не сбрасывается и не меняется при переходе между неделями.
  const weekDates = Array.from({ length: 7 }, (_, i) => { const d = new Date(); d.setDate(d.getDate() - ((d.getDay() + 6) % 7) + i); return d })
  ...
  const addTransaction = () => { const amount = Number(transaction.amount); if (transaction.amount.trim() === '' || amount < 0) return; update({ transactions: [...data.transactions, { id: Date.now(), type: transaction.type, category: transaction.category, amount, month: financeMonth, date: transaction.date }] }); setTransaction({ type: transaction.type, category: transaction.category, amount: '', date: dateKey(0) }); setShowTransaction(false) }
