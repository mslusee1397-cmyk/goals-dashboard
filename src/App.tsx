        ...
        <Card title="Финансы" eyebrow="ДОХОДЫ И РАСХОДЫ" icon={<Wallet size={22} />} className="finance-card">
          ...
          <div className="finance-lower">
            <div className="transactions-list">
              ...
              {monthTransactions.length === 0 ? <div className="empty-transactions">Пока нет операций. Добавь первый доход или расход кнопками выше.</div> : monthTransactions.slice().sort((a,b) => b.date.localeCompare(a.date)).slice(0,6).map(item => <div className="finance-transaction" key={item.id}><span className={item.type === 'income' ? 'transaction-icon income' : 'transaction-icon expense'}>{item.type === 'income' ? <ArrowUpRight size={16} /> : <ArrowDownLeft size={16} />}</span><div><b>{item.category}</b><small>{new Date(`${item.date}T12:00:00`).toLocaleDateString('ru-RU')}</small></div><strong className={item.type === 'income' ? 'income' : 'expense'}>{item.type === 'income' ? '+' : '−'}{item.amount.toLocaleString('ru-RU')} ₽</strong></div>)}
            </div>
